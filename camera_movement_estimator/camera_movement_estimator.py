...
class CameraMovementEstimator:
    ...
    def get_camera_movement(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num in range(1, len(frames)):
            ...
            if max_distance > self.mininum_distance:
    ...
    def draw_camera_movement(self, frames, camera_movement_per_frame):
        ...
        for frame_num, frame in enumerate(frames):
            ...
            frame = cv2.putText(frame, f"Camera Movement X :{movement_x:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
            frame = cv2.putText(frame, f"Camera Movement Y :{movement_y:.2f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
